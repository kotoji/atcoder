...
int main() {
  ...
  int maxp =
      accumulate(all(ps), 0, [](int acc, auto p) { return acc + p.second; });

  vec<pair<vec<char>, int>> queue;
  {
    // 1個除くとき
    rep(i, 0, 5) {
      vec<int> v(1);
      v[0] = i;
      int point = maxp - ps[i].second;
      vec<char> out;
      rep(i, 0, 5) {
        if (find(all(v), i) == v.end()) {
          out.push_back(ps[i].first);
        }
      }
      sort(all(out));
      queue.push_back({out, point});
    }
  }
  {
    // 2個除くとき
    rep(i, 0, 5) {
      rep(j, i + 1, 5) {
        ...
        v[0] = i;
        v[1] = j;
        int point = maxp - ps[i].second - ps[j].second;
        vec<char> out;
        rep(i, 0, 5) {
          if (find(all(v), i) == v.end()) {
            out.push_back(ps[i].first);
    ...
    rep(i, 0, 5) {
      rep(j, i + 1, 5) {
        rep(k, j + 1, 5) {
          ...
          v[1] = j;
          v[2] = k;
          int point = maxp - ps[i].second - ps[j].second - ps[k].second;
          vec<char> out;
          rep(i, 0, 5) {
            if (find(all(v), i) == v.end()) {
              out.push_back(ps[i].first);
